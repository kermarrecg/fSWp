...
for i=1:NN
    ...
    for j=1:mm 
        if mD(j,i)<0.5; mS(j,i) = log( ((gamma(1-mD(j,i))^2)/(gamma(1-2*mD(j,i))))* (1-mD(end,i))*nanvar(vy0))*0.5; else
            mS(j,i) = log(  (1-mD(end,i)) *nanvar(vy0))*0.5; end
    end
 
 vLL(i)=-f_LLik2( [ log(mD(1:mm,i)./(1-mD(1:mm,i) ));... 
      mS(1:end,i);   log(  ( mD(end,i)) *nanvar(vy0))*0.5 ;1.3]);%;   log( 0.01*nanvar(vy))*0.5 ]); %; log(  0.2*nanvar(vy))*0.5 ]);
