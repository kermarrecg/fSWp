...
vVV=[0.0000 ,   1.0000 ,   0.5000  ,  0.6000 ,   0.3000  ,  0.4000   , 0.4000   , 0.6000];v2=0;
...
jjj=1
structData = readtable(['DRAO_PWV.txt'],'FileType','text'); ID=  'DRAO'
   ...
   if BC==1
   [v1,v2]= f_fSW_BoxCox(vr, dLambda, cp, mX);
   %v1=vVV(jjj);
   if v1<0.001
       vy=log(vr);
   else
...
%     figure();histogram(vy);
    ...
    dphi_trend_hat= vP_hat(2*cCyc+2)/sqrt(1 + vP_hat(2*cCyc+2)^2)
    %dsigma2_zeta_hat = exp(2*vP_hat(1+3*cCyc))
    %dd_trend_hat=exp(vP_hat(2*cCyc+2))./(1+exp(vP_hat(2*cCyc+2)))
    ...
    mPar(:,jjj)=[ vBeta(1:2)' vd_hat' vSigma2_eta_hat' dsigma2_eta0_hat dphi_trend_hat]    ;
...
%     SmoothEst=mArt_smooth([1:(cp+1):( 1+2*cCyc)*(cp+1)],:);
%     SmoothEst_var=mPrt_smooth([1:(cp+1):( 1+2*cCyc)*(cp+1)],:);
...
%     vy_hat=mX*vBeta +sum(SmoothEst(1:2,:)',2) + sum(mX(:,[2:end]).* SmoothEst(3:end,:)',2);
    ...
    if BC==1
    [vTrend,v1stCyc,v2ndCyc,vTrend_var,v1stCyc_var,v2ndCyc_var]=fSimSmoother(vy,vP_hat,dLambda,mX,vBeta,cp,v1,1000);
    end
    ...
    orient(g1,'landscape')
    % print(g1,strcat('ID','_RedNoise_',string(jjj)),'-dpdf', '-fillpage', '-painters','-r600')

%     g1=figure()
%     plot(vtime,vBeta(1) + SmoothEst(1,:)' ,'b','LineWidth',2)
%     hold on
%     plot(vtime,vBeta(1) + SmoothEst(1,:)'  + 1.96*sqrt(SmoothEst_var(1,:))','g--','LineWidth',2)
%     plot(vtime,vBeta(1) + SmoothEst(1,:)' - 1.96*sqrt(SmoothEst_var(1,:))','g--','LineWidth',2)
%     title(strcat(ID,'Trend component Series and 95% Confidence Intervals',string(jjj)))
%     orient(g1,'landscape')
%     print(g1,strcat(ID,'Trend and CIs',string(jjj)),'-dpdf', '-fillpage', '-painters','-r600')
...
%     plot(vtime, v1stCyc +1.96*sqrt(v1stCyc_var),'g','LineWidth',1.3)
%     plot(vtime, v1stCyc -1.96*sqrt(v1stCyc_var),'g','LineWidth',1.3)
    ...
    if harm>1
        ...
        %         plot(vtime, v2ndCyc +1.96*sqrt(v2ndCyc_var),'g','LineWidth',1.3)
        %         plot(vtime, v2ndCyc -1.96*sqrt(v2ndCyc_var),'g','LineWidth',1.3)
    ...
    title('Amplitude 1st Cyclical component of the trasformed series')
    xlim([vtime(1) vtime(end)])
    subplot(2,1,2)
    plot(vtime,atan((SmoothEst2(2,:)-vBeta2(2))./(SmoothEst2(1,:)-vBeta2(1))) ,'r')
    title('Phase 1st Cyclical component of the trasformed series')
    xlim([vtime(1) vtime(end)])
    orient(g1,'landscape')
    print(g1,strcat('ID','_Amp_',string(jjj)),'-dpdf', '-fillpage', '-painters','-r600')
    if harm>1
        g1=figure()
        subplot(2,1,1)
        plot(vtime,sqrt((SmoothEst2(3,:)-vBeta2(3)).^2  + (SmoothEst2(4,:)-vBeta2(4)).^2) ,'r')
        title('Amplitude 2nd Cyclical component of the trasformed series')
        xlim([vtime(1) vtime(end)])
        subplot(2,1,2)
        plot(vtime,atan((SmoothEst2(4,:)-vBeta2(4))./(SmoothEst2(3,:)-vBeta2(3))) ,'r')
        title('Phase 2nd Cyclical component of the trasformed series')
        xlim([vtime(1) vtime(end)])
        orient(g1,'landscape')
        print(g1,strcat('ID','_2ndAmp_',string(jjj)),'-dpdf', '-fillpage', '-painters','-r600')
    end
