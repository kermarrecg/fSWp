function [vP0,vLL]= fgrid_InitCondsBN(vy, dLambda , mXreg,cp )
mm= length( dLambda  ) ;
m2=mm ;

vd0=linspace(.14,.64,5);
vd1=linspace(.14,.64,5);
vr=linspace(0.001,0.05,5);
...
NN=size(mD,2);
vLL=NaN(NN,1);
f_LLik2= @(theta)   fSsfLogLikWTZR(theta, vy, dLambda,mXreg, cp) ;  %%Log likelihood
mS=NaN(mm,NN);
%vthetaphi0=NaN(NN,1);
for i=1:NN
    for j=1:mm 
