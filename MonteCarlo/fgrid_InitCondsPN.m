function [vP0,vLL]= fgrid_InitCondsPN(vy, dLambda , mX,cp  )
mm= length( dLambda  ) ;
m2=mm+1;

vd0=linspace(.65,.95,5);
vd1=linspace(.25,.65,5);
vr=linspace(0.01,0.5,5);
...
for i=1:NN
    ...
    for j=1:mm 
        if mD(j,i)<0.5; mS(j,i) = log( ((gamma(1-mD(j,i))^2)/(gamma(1-2*mD(j,i))))* (1-mD(end,i)) *nanvar(vy))*0.5; else
            mS(j,i) = log(  (1-mD(end,i)) *0.1*nanvar(vy))*0.5; end
    end
 
 vLL(i)=-f_LLik2( [ log(mD(1:mm,i)./(1-mD(1:mm,i) ));... 
      mS(1:end,i);   log(  ( mD(end,i)) *0.1*nanvar(vy))*0.5;  log(mD(end-1,i)./(1-mD(end-1,i) )) ...
       ]   );
