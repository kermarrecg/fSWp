...
for jjj= 11  %, 3:7]%1:
    ...
    if jjj==1; structData = readtable(['ADE1_PWV.txt']); ID=  'ADE1';end
    if jjj==2; structData = readtable(['ADIS_PWV.txt']); ID=  'ADIS';end
    if jjj==3; structData = readtable(['AIRA_PWV.txt']); ID=  'AIRA';end
    if jjj==4; structData = readtable(['ALBH_PWV.txt']); ID=  'ALBH';end
    if jjj==5; structData = readtable(['ALGO_PWV.txt']); ID=  'ALGO';end
    if jjj==6; structData = readtable(['ALIC_PWV.txt']); ID=  'ALIC';end
    if jjj==7; structData = readtable(['ALRT_PWV.txt']); ID=  'ALRT';end
    if jjj==8; structData = readtable(['AMC2_PWV.txt']); ID=  'AMC2';end
    if jjj==9; structData = readtable(['ANKR_PWV.txt']); ID=  'ANKR';end
    if jjj==10; structData = readtable(['AOML_PWV.tab'],'FileType','text'); ID=  'AOML';end
    if jjj==11; structData = readtable(['DRAO_PWV.txt']); ID=  'DRAO';end
...
%     figure();histogram(vy);
    ...
    %Initial consitions
    dphi0=1.3/sqrt(1 +1.3^2);
    vP0=[ log(  (nanvar(vr)/(1-dphi0^2)))*0.5 ;1.3  ];

    % maximization of the log likelihood

    fU  = @(vP)  fSsfLogLik_Harvey(vP, vy, mX); % function
    ...
    dsigma2_eta0_hat = exp(2*vP_hat(1 ))
    dphi_trend_hat= vP_hat( 2)/sqrt(1 + vP_hat(2)^2)
 

    % Exatract all the components
    [mZt, mGG, mT, mHH, va, mP ] = SetStateSpaceModel_Harvey(vP_hat ,cn);
    [mArt_smooth, mPrt_smooth, vBeta, mVarBeta, aP] = fAugmentedStateSmoother_Harvey(vy, mZt, ....
        mGG, mT, mHH, va, mP, [], mX);

    mPar(:,jjj)=[ vBeta'   dsigma2_eta0_hat dphi_trend_hat]    ;

    [mZts, mGGs, mTs, mHHs, vas, mPs ] = SetStateSpaceModel_ExactLik(vP_hat,mX,vBeta,cn);
