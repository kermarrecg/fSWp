function [vTrend,v1stCyc,v2ndCyc,vTrend_var,v1stCyc_var,v2ndCyc_var]=fSimSmoother(vy,vP_hat,dLambda,mX,vBeta,cp,v1,MC)
...
if v1>0.001
    ...
    vTrend_MC2= vTrend_MC2       + (1 + v1*(mX(:,1:2)*vBeta(1:2) + SmoothEst(1,:)')).^(2/v1);
    v1stCyc_MC =  v1stCyc_MC   + (1 + v1*(  mX(:,1:4)*vBeta(1:4) +  sum(mX2(:,1:2).* SmoothEst2(1:2,:)',2) )).^(1/v1);
    v1stCyc_MC2 =  v1stCyc_MC2   + (1 + v1*( mX(:,1:4)*vBeta(1:4) +  sum(mX2(:,1:2).* SmoothEst2(1:2,:)',2) )).^(2/v1);
    v2ndCyc_MC =  v2ndCyc_MC   + (1 + v1*( mX(:,[1,2,5,6])*vBeta([1,2,5,6])  +  sum(mX2(:,3:4).* SmoothEst2(3:4,:)',2))).^(1/v1);
    v2ndCyc_MC2 =  v2ndCyc_MC2   + (1 + v1*( mX(:,[1,2,5,6])*vBeta([1,2,5,6])   +  sum(mX2(:,3:4).* SmoothEst2(3:4,:)',2))).^(2/v1); 
else
    vTrend_MC=    vTrend_MC    + exp( mX(:,1:2)*vBeta(1:2) + SmoothEst(1,:)') ;
    vTrend_MC2=    vTrend_MC2    + exp( mX(:,1:2)*vBeta(1:2) + SmoothEst(1,:)').^2 ;
    v1stCyc_MC =  v1stCyc_MC   + exp( (mX(:,1:4)*vBeta(1:4)  +  sum(mX2(:,1:2).* SmoothEst2(1:2,:)',2) )) ;
    v1stCyc_MC2 =  v1stCyc_MC2   + exp( ( mX(:,1:4)*vBeta(1:4)  +  sum(mX2(:,1:2).* SmoothEst2(1:2,:)',2) )).^2 ;
    v2ndCyc_MC =  v2ndCyc_MC   + exp( (mX(:,[1,2,5,6])*vBeta([1,2,5,6])  +  sum(mX2(:,3:4).* SmoothEst2(3:4,:)',2))) ;
    v2ndCyc_MC2 =  v2ndCyc_MC2   + exp( ( mX(:,[1,2,5,6])*vBeta([1,2,5,6])  +  sum(mX2(:,3:4).* SmoothEst2(3:4,:)',2))).^2 ;
end
